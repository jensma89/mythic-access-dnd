...
def read_diceset(
        ...
        diceset_id: int = Path(
            ...,
            description="The ID of dice set to retrieve."
        ),
...
def update_diceset(
        ...
        diceset_id: int = Path(
            ...,
            description="The ID of dice set to update."
        ),
        current_user: User = Depends(get_current_user),
        service: DiceSetService = Depends(get_diceset_service)
):
    ...
    if existing_diceset.user_id != current_user.id:
        logger.warning(
            ...
            f"to update dice set {diceset_id} "
            f"not owned by them"
        )
        raise HTTPException(
            status_code=403,
    ...
    logger.info(
        f"PATCH update dice set {diceset_id} "
        f"by user {current_user.id}"
    )
...
def delete_diceset(
        ...
        diceset_id: int = Path(
            ...,
            description="The ID of dice set to delete."
        ),
        current_user: User = Depends(get_current_user),
        service: DiceSetService = Depends(get_diceset_service)
):
    ...
    if existing_diceset.user_id != current_user.id:
        logger.warning(
            f"User {current_user.id} tried "
            f"to delete dice set {diceset_id} "
            f"not owned by them"
        )
        raise HTTPException(
            status_code=403,
...
def roll_diceset(
        ...
        diceset_id: int = Path(
            ...,
            description="The ID of the dice set to roll"
        ),
    ...
    if not diceset:
        logger.warning(
            f"Dice set {diceset_id} not found "
            f"before roll attempt"
        )
        raise HTTPException(
            status_code=404,
            detail="Dice set not found."
    ...
    if diceset.user_id != current_user.id:
        logger.warning(
            ...
            f"ROLL dice set {diceset_id} "
            f"owned by {diceset.user_id}"
        )
        raise HTTPException(
            status_code=403,
            detail="Not allowed"
