...
class DiceSetService:
    ...
    def get_diceset(
        ...
        try:
            ...
            if not db_diceset:
                logger.warning(f"DiceSet {diceset_id} "
                               f"not found")
                raise HTTPException(
                    status_code=404,
            ...
            logger.exception(
                f"Database error "
                f"while fetching DiceSet {diceset_id}",
                exc_info=True
            )
            raise HTTPException(
                status_code=500,
    ...
    def update_diceset(
        ...
        try:
            ...
            logger.exception(
                f"Database error "
                f"while updating DiceSet {diceset_id}",
                exc_info=True
            )
            raise HTTPException(
                status_code=500,
    ...
    def delete_diceset(
        ...
        try:
            ...
            logger.exception(
                f"Database error "
                f"while deleting DiceSet {diceset_id}",
                exc_info=True
            )
            raise HTTPException(
                status_code=500,
    ...
    def _log_roll(
        ...
        try:
            ...
            logger.exception(
                f"Database error "
                f"while logging roll for DiceSet {diceset_id}",
                exc_info=True
            )
            raise HTTPException(
                status_code=500,
    ...
    def roll_diceset(
        ...
        try:
            ...
            logger.info(f"Rolled DiceSet {diceset_id} "
                        f"by User {user_id}: "
            ...
            logger.exception(
                f"Database error "
                f"while rolling DiceSet {diceset_id}",
                exc_info=True
            )
            raise HTTPException(
                status_code=500,
